...
# Load portfolio data
portfolio_data = pd.read_excel(file_path, sheet_name='Sheet1', usecols=['TICKER', 'ALLOCATION'])
tickers = portfolio_data['TICKER'].tolist()  
weights = portfolio_data['ALLOCATION'].values  

# Load portfolio value
portfolio_value_df = pd.read_excel(file_path, sheet_name='Sheet1', usecols='D', nrows=1)
...
# Generate random daily returns outside the jitted function
random_returns = np.random.multivariate_normal(mean_returns_np, cov_matrix_np, (num_simulations, num_days))

# Jitted function for Monte Carlo simulation
@jit(nopython=True, parallel=True)
def test(random_returns, weights, portfolio_value):
...
# Run the simulation with random returns passed to the function
simulation_results, portfolio_highs, portfolio_lows = test(random_returns, weights, portfolio_value)
